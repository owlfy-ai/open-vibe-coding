...
/**
 * Compact inline SVG icon set (stroke-based, 24x24, currentColor).
 * No external dependency. Fill is opt-in per-icon via `fill="currentColor"`.
 */
const ICONS = {
  ...
  pin: <path d="M12 2 22 12 12 22 2 12Z" />,
  pinFilled: <path d="M12 2 22 12 12 22 2 12Z" fill="currentColor" stroke="none" />,
  more: (
    <>
  ...
  pencil: (
    <>
      <path d="M21.17 6.81a1 1 0 0 0-3.98-3.99L3.84 16.17a2 2 0 0 0-.5.83l-1.32 4.36a.5.5 0 0 0 .62.62l4.36-1.32a2 2 0 0 0 .83-.5z" />
      <path d="m15 5 4 4" />
    </>
  ),
  search: (
...
const TOOL_ICONS: Readonly<Record<string, IconName>> = {
  ...
  manage_dependencies: "package",
  get_console_logs: "terminal",
  web_search: "search",
  web_reader: "globe",
  image_search: "image",
  search_npm_packages: "package",
  get_npm_package_detail: "package",
  manage_memories: "database",
};

export function ToolIcon({ name, size = 15 }: { readonly name: string; readonly size?: number }) {
  return <Icon name={TOOL_ICONS[name] ?? "command"} size={size} />;
}
